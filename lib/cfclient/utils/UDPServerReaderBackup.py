...
Class UDPServerReader is the copy of PyGameReader class, whith same variables and methods, and communicates with UDPServer.
...
class UDPServerReader():
	...
	def start_input(self, deviceId, inputMap, port):
		...
		self.data = {"roll":0.0, "pitch":0.0, "yaw":0.0, "yaw_Opti":0.0, "thrust":0.0, "pitchcal":0.0, "rollcal":0.0, "estop": False, "exit":False, "althold":False}
	...
	def read_input(self):
		...
		"""
		roll = float(roll) / (math.pi/4)          # Input range [-pi/4, pi/4] converted to [-1, 1]
		pitch = float(pitch) / (math.pi/4)        # Input range [-pi/4, pi/4] converted to [-1, 1]
		...
		#if self.contatore > 20:
		#	self.contatore = 0
		#	print -yaw
		#	sys.stdout.flush()
		
		
		self.data["roll"], self.data["pitch"], self.data["yaw"], self.data["thrust"], self.data["yaw_Opti"] = roll, pitch, -yaw, thrust, -yaw_Opti
		"""
		self.i += 1
		if (self.i > 100):
			print self.data
